---
name: design-md
description: Give any AI-built project a real design system by authoring or importing a DESIGN.md — a single AI-readable file carrying the theme, color tokens, typography scale, spacing, components and motion rules that the agent must follow on every edit. Use when the user says "make it look professional", "it looks like every other AI site", "use this design", "match this site's style", "create a design system", "write a DESIGN.md", or pastes a link from styles.refero.design or motionsites.ai. Also use at project start, before any redesign, and whenever visual drift appears between pages.
---
...
| `references/example-crt.md` | Complete worked example (green CRT terminal style) |
