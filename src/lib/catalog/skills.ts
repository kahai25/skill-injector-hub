...
export const SKILLS: Skill[] = [
  ...
  },
  {
    ...
    name: "webapp-testing",
    purpose:
      "Anthropic's official Playwright toolkit skill: drive and test local web apps, verify frontend behaviour, capture browser screenshots and read browser logs from native Python scripts.",
    categories: ["planning", "performance"],
    platforms: ["claude-code", "cursor", "lovable"],
    triggers: [
      "test my web app",
      "write a Playwright script",
      "screenshot the page and check it",
      "read the browser console logs",
    ],
    author: anthropicAuthor,
    repo: anthropicRepo("webapp-testing"),
  ...
  },
  {
    ...
    name: "skill-creator",
    purpose:
      "Anthropic's official skill for authoring skills: create one from scratch, edit or optimize an existing skill, run evals, benchmark performance with variance analysis, and tune descriptions for better triggering.",
    categories: ["planning"],
    platforms: ["claude-code", "cursor", "lovable"],
    triggers: [
      "create a new skill",
    ...
    author: anthropicAuthor,
    repo: anthropicRepo("skill-creator"),
    featuredExternal: true,
    files: [
      "SKILL.md",
      "LICENSE.txt",
  ...
  },
  {
    ...
    name: "design-md",
    purpose:
      "Author or import a DESIGN.md — one AI-readable file carrying theme, color tokens, type scale, spacing, components and motion rules the agent follows on every edit. Includes a template, three worked examples spanning a flat green CRT terminal, a motion-heavy black WebGL immersive studio, and a loud anti-corporate sticker-pop editorial style, plus curated free/paid gallery sources.",
    categories: ["design"],
    platforms: ["lovable", "claude-code", "cursor"],
    triggers: [
